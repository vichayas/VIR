...
BEGIN TRY
	BEGIN TRANSACTION
		...
		IF (@Subclass = '511' or  @Subclass = '515')
			select @refNo = dbo.GenReferenceNumber(@runningNumber,0,1,@BranchCode,@Subclass,@yearAD)
		ELSE
			select @refNo = dbo.GenReferenceNumber(@runningNumber,0,0,@BranchCode,@Subclass,@yearAD)
		...
		left join CompanyRegistration f on (a.Party_Id = f.Organization_Id and f.[Type_Id] = 'F8E30A8A-00DE-4A77-AD47-10CF35F923F4') --เลขผู้เสียภาษี
