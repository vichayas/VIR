...
declare @longcurdate nvarchar(16)
declare @NotInlongcurdate nvarchar(16)
--yyyymmdd
set @curdate=CONVERT(char(8),getdate(),112)
set @longcurdate=CONCAT(@curdate, ' 10:20')
set @NotInlongcurdate=CONCAT(CONVERT(char(11),getdate(),121),'10:2')
print @NotInlongcurdate
print @longcurdate
...
begin
	...
	set @sql='insert into #tablename select Name = '''+@t_name +''', TDate = convert(char(19),max('+@c_name+'),121), count(1) from ['+@t_name +'] with (nolock) where convert(char(16),'+@c_name+',121)>'''+@longcurdate+''' AND convert(char(15),CreatedDate,121)!='''+@NotInlongcurdate+'''  group by convert(char(8),'+@c_name+',112) having convert(char(8),'+@c_name+',112)='''+@curdate+''' and max('+@c_name+') > 0'
	print @sql
