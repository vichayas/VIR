...
WHERE OBJECT_NAME (f.referenced_object_id) = 'AgentCommissionRate'
Order by table_name


set @item=0

create table #tablename (t_name nvarchar(100))
...
begin
	...
	set @sql='insert into #tablename select Name = '''+@t_name +''' from ['+@t_name +'] with (nolock) where '+@c_name+' = ''AAB5395C-7D46-4A63-9C60-ECBADC7A13D1'''
...
select distinct t_name  from #tablename with (nolock) order by t_name desc
